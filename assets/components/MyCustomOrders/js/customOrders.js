$(document).ready(function() {
	...
	target.click(function(event) {
        ...
        success: function(response) {
        	...
        	if (response.order[0].payment == 1) {
        		modal.find('#payment_name').text('Сбербанк');
        	} else {
        	...
        	modal.find('#delivery_adress').text(response.order[0].index + ', ' + response.order[0].region + ', ' + response.order[0].city + ', ' + response.order[0].street + ', д.' + response.order[0].building + ', к.' + response.order[0].properties.extfld_housing + ', кв.' + response.order[0].room);
